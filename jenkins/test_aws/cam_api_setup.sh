...
export CAM_BEARER_TOKEN=$(curl -k -X POST -H "Content-Type: application/x-www-form-urlencoded;charset=UTF-8" -d "grant_type=password&username=${CAM_USER_CREDS_USR}&password=${CAM_USER_CREDS_PSW}&scope=openid" ${ICP_URL}/idprovider/v1/auth/identitytoken 2> /dev/null | python -c "import sys, json; print json.load(sys.stdin)['access_token']")
